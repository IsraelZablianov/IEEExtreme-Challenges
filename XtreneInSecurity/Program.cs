...
namespace XtreneInSecurity
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> already = new List<string>(1000);

            for (int i = 0; i < LogList.Count; i++)
            {
                if (!already.Contains(LogList[i].Trim()) && LogList[i].Trim() != "")
                {
                    already.Add(LogList[i].Trim());
                    map[getEncodedWord(LogList[i].Trim())] = LogList[i].Trim();
                }
            }
            using (StreamWriter file = new StreamWriter("myfile.txt"))
            {
                ...
                {
                    file.WriteLine("map[\"{0}\"]=\"{1}\";", entry.Key.Trim(), entry.Value.Trim());
                }
            }
